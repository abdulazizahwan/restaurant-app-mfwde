import FavoriteRestoIdb from '../data/favoriteresto-idb';
import Home from './home';

const Favorite = {
  ...Home,
  async render() {
    const restaurants = await FavoriteRestoIdb.getAll();
    this.restaurans = restaurants;
    return `
        <section class="hero home-jumbotron with-default-bg" tabindex="0">
            <div class="hero-overlay">
                <div class="hero-content">
                    <div class="caption">
                        <span>Restoran mana aja yang Favorit?</span>
                        <p>Menampilkan daftar RestoranQue. yang disukai!
                        </p>
                    </div>
                </div>
            </div>
        </section>
        <div class="container" id="content" tabindex="0">
            <h1 class="p-1 pl-2 pt-2" tabindex="0">Restoran Favorit</h1>

            <div class="p-1">
                <div class="list-restaurant" id="list-restaurant"></div>
                <div id="list-restaurant-zero"></div>

            </div>
        </div>
    `;
  },
  notAvailableMessage() {
    return `
            <div class="flex justify-center pt-3 pb-3">
                <div class="block text-center">
                    <i class="fas fa-frown fa-3x block"></i>
                    <p class"mt-1">Daftar <strong>Restoran Favorit</strong> anda masih kosong. ☹️</p>
                </div>
            </div>
        `;
  },
};

export default Favorite;
